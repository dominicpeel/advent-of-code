...
for a in range(len(galaxies)):
    ...
    for b in range(a+1, len(galaxies)):
        ...
        row_expansions_2 = sum(row in clear_rows for row in range(*sorted((ai, bi))))
        col_expansions_2 = sum(col in clear_cols for col in range(*sorted((aj, bj))))

        p1 += row_expansions + col_expansions + manhattan(galaxies[a], galaxies[b])
        p2 += (1000000-1)*(row_expansions_2 + col_expansions_2) + manhattan(galaxies[a], galaxies[b])
